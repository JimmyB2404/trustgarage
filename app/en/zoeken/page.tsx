...
import { SERVICES, LANGUAGES, SERVICE_LABELS_EN, LANGUAGE_LABELS_EN } from '@/lib/mock-data'
...
function ZoekenContentEn() {
  ...
  // Track only after 600ms of silence — avoids one event per keystroke.
  useEffect(() => {
  ...
  const FilterPanel = () => (
    <>
      ...
      <label className="flex items-center gap-2 cursor-pointer mb-6">
        ...
        />
        <span className="text-[13px] text-neutral-900">Open now</span>
      </label>
        ...
        {!userLocation ? (
          ...
          <div>
            ...
            <p className="text-[12px] text-neutral-500 mt-1">
              {maxDistance >= MAX_DISTANCE_LIMIT ? 'No limit' : `Within ${maxDistance} km`}
            </p>
          </div>
        )}
        {locationStatus === 'denied' && (
          <p className="text-[12px] text-danger mt-1">Location not available.</p>
        )}
      </div>
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <input
          ...
          className="w-[14px] h-[14px] accent-primary cursor-pointer"
        />
        <span className="text-[13px] text-neutral-900">KVK verified only</span>
      </label>
    </>
  )
          ...
          <div className="max-w-site mx-auto px-4 sm:px-6">
            <div className="flex gap-3 items-center">
              ...
              <div className="hidden lg:flex items-center gap-1.5 relative">
                <label className="text-[12px] text-neutral-500 whitespace-nowrap">Sort by:</label>
              ...
              <div className="flex flex-wrap gap-2 mt-2.5">
                {openNowOnly && (
                  <span className="inline-flex items-center gap-1 text-[12px] bg-primary-light text-primary px-2.5 py-1 rounded-full">
                    Open now
                    <button onClick={removeOpenNow} aria-label="Remove open-now filter">
                      <IconX size={12} />
                    </button>
                  </span>
                )}
                {distanceActive && (
                  <span className="inline-flex items-center gap-1 text-[12px] bg-primary-light text-primary px-2.5 py-1 rounded-full">
                    ...
                    <button onClick={removeDistance} aria-label="Remove distance filter">
                      <IconX size={12} />
                    </button>
                  </span>
                )}
                ...
                {selectedLanguages.map(l => (
                  <span
                    ...
                    className="inline-flex items-center gap-1 text-[12px] bg-primary-light text-primary px-2.5 py-1 rounded-full"
                  >
                    {LANGUAGE_LABELS_EN[l] ?? l}
                    <button onClick={() => removeLanguage(l)} aria-label={`Remove ${l}`}>
                      <IconX size={12} />
                    </button>
                  </span>
                ))}
                {minRating > 0 && (
                  <span className="inline-flex items-center gap-1 text-[12px] bg-primary-light text-primary px-2.5 py-1 rounded-full">
                    {minRating}+ stars
                    <button onClick={removeMinRating} aria-label="Remove minimum rating">
                      <IconX size={12} />
                    </button>
                  </span>
                )}
                {kvkOnly && (
                  <span className="inline-flex items-center gap-1 text-[12px] bg-primary-light text-primary px-2.5 py-1 rounded-full">
                    KVK verified
                    <button onClick={removeKvk} aria-label="Remove KVK filter">
                      <IconX size={12} />
                    </button>
                  </span>
                )}
        ...
        <div className="max-w-site mx-auto px-4 sm:px-6 py-6 flex gap-6">
          ...
          <main className="flex-1 min-w-0">
            <p className="text-[12px] text-neutral-500 mb-3 flex items-center gap-1.5">
              ...
              Reviews with a &quot;Verified visit&quot; badge are tied to a confirmed visit —
              something Google Reviews doesn&apos;t offer.
            </p>
            {/* Results header */}
            <div className="flex items-center justify-between mb-4">
              <p className="text-[13px] text-neutral-500">
                <span className="font-medium text-neutral-900">{filteredGarages.length}</span>{' '}
                {filteredGarages.length === 1 ? 'garage found' : 'garages found'}
              </p>
              {/* Mobile sort + list/map toggle */}
            ...
            {/* Mobile map view — fully replaces the list, no split like on desktop */}
            {mobileView === 'map' && (
              <div className="lg:hidden h-[calc(100dvh-260px)] mb-4">
                <SearchResultsMap garages={filteredGarages} locale="en" />
              </div>
            )}
            ...
            {mobileView === 'map' ? null : paginatedGarages.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-[16px] font-medium text-neutral-900 mb-1">No garages found</p>
          ...
          {/* Map — desktop: permanent, sticky column next to the list */}
      ...
      {compareIds.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-neutral-900 text-white px-4 py-3 z-[60] flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-[13px]">
            ...
            <span>{compareIds.length} of {MAX_COMPARE} selected to compare</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => router.push(`/en/compare?ids=${compareIds.join(',')}`)}
...
export default function ZoekenPageEn() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-surface flex items-center justify-center text-neutral-500">Loading...</div>}>
      <ZoekenContentEn />
    </Suspense>
  )
}
